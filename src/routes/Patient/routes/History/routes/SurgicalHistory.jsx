...
function SurgicalHistoryDetailPanel({ row, onSave, onCancel, onDelete }) {
...
export default function SurgicalHistory() {
  ...
  const handleDelete = (id) => {
    setSurgicalHx((prev) => prev.filter((row) => row.id !== id));
  };
  ...
  const handleCancel = (row) => {
    if (row.isNew) {
      setSurgicalHx((prev) => prev.filter((r) => r.id !== row.id));
    }
    setExpandedRowIds(new Set());
  };
  ...
  const columns = [
    { field: 'procedure', headerName: 'Procedure', flex: 1, renderCell: (params) => <Box sx={{ color: '#e91e63', fontWeight: 'bold' }}>{params.value}</Box> },
    ...
    <TitledCard emphasized title={<><Icon sx={{ verticalAlign: "text-top", mr: "4px" }}>token</Icon> Surgical History</>} color="#9F3494">
      ...
      <Box sx={{ height: 500, width: '100%' }}>
        <DataGrid
          rows={surgicalHx ?? []}
          columns={columns}
          getRowId={(row) => row.id || `surgical-${row.procedure}-${row.date}-${row.age}`}
