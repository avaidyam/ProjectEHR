...
const CareTeamDialog = ({ open, onClose, careTeam, setCareTeam, allProviders }: {
    ...
    const handleAddMember = () => {
        if (newProviderId && newProviderRole) {
            if (localCareTeam.some((m: any) => m.provider === newProviderId)) {
    ...
    const handleRemoveMember = (providerId: string) => {
        setLocalCareTeam(localCareTeam.filter((m: any) => m.provider !== providerId));
    };

    const handleRoleChange = (providerId: string, newRole: string) => {
        setLocalCareTeam(localCareTeam.map((m: any) =>
            m.provider === providerId ? { ...m, role: newRole } : m
        ));
    };

    const availableProviders = allProviders.filter((p: any) => !localCareTeam.some((m: any) => m.provider === p.id));
    ...
    return (
        ...
        >
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                {localCareTeam.map((member: any) => {
                    const provider = allProviders.find((p: any) => p.id === member.provider);
                    return (
                        <Box key={member.provider} sx={{ display: 'flex', alignItems: 'center', gap: 2, p: 1, border: '1px solid #eee', borderRadius: 1 }}>
                            ...
                            <TextField
                                ...
                                size="small"
                                value={member.role}
                                onChange={(e: any) => handleRoleChange(member.provider, e.target.value)}
                                sx={{ width: 150 }}
                            />
                ...
                <Box sx={{ display: 'flex', gap: 2, alignItems: 'flex-start' }}>
                    <FormControl fullWidth size="small">
                        ...
                        <Select
                            value={newProviderId}
                            label="Provider"
                            onChange={(e: any) => setNewProviderId(e.target.value)}
                        >
                    ...
                    <TextField
                        ...
                        onChange={(e: any) => setNewProviderRole(e.target.value)}
                    />
