...
//       );
//     case 'stickyNotes':
//       return patient.stickyNotes?.private ? String(patient.stickyNotes.private).trim() : '';
//     default:
...
//   { id: 'stickyNotes', label: 'Sticky Notes', selected: true, order: 5 },
// ];
...
const getColumnValue = (patient, columnId) => {
  switch (columnId) {
    ...
    case 'location':
      return (
        <Box sx={{ display: 'flex', flexDirection: 'column' }}>
          <Typography variant="body2">{patient.location}</Typography>
          {patient.bedStatus && (
            <Typography variant="caption" color="text.secondary">
              {patient.bedStatus}
            </Typography>
...
export const PatientsTable = () => {
  ...
  useEffect(() => {
    if (selectedList?.patients?.length) {
      setReady(true);
    }
  }, [selectedList]);
