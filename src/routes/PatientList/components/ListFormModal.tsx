...
export const ListFormModal = ({
    ...
    open: boolean;
    onClose: () => void;
    onSubmit: (name: string, columns: any[]) => void;
    initialData?: any;
}) => {
    const isEditMode = !!initialData;
    const [listName, setListName] = React.useState('');
    const [selectedColumns, setSelectedColumns] = React.useState<any[]>([]);
    const [unselectedColumns, setUnselectedColumns] = React.useState<any[]>([]);
    ...
    React.useEffect(() => {
        if (open) {
            if (isEditMode) {
                ...
                const existingColumnIds = new Set(initialData.columns.map((col: any) => col.id));
                ...
                setUnselectedColumns(
                    availableColumns
                        .filter((col: any) => !existingColumnIds.has(col.id))
                        .sort((a: any, b: any) => a.order - b.order)
                );
            } else {
    ...
    const handleAddColumn = (column: any) => {
        setSelectedColumns([...selectedColumns as any[], column]);
        setUnselectedColumns((unselectedColumns as any[]).filter((c: any) => c.id !== column.id));
    };

    const handleRemoveColumn = (column: any) => {
        setUnselectedColumns(
            [...unselectedColumns as any[], column].sort((a, b) => a.order - b.order)
        );
        setSelectedColumns((selectedColumns as any[]).filter((c: any) => c.id !== column.id));
    };

    const handleMoveColumn = (index: number, direction: 'up' | 'down') => {
        const newColumns = [...selectedColumns as any[]];
    ...
    const handleSubmit = () => {
        const formattedColumns = selectedColumns.map((col: any, index: number) => ({
            ...col,
            selected: true,
            order: index,
        }));
                    ...
                    <Box sx={{ flex: 1 }}>
                        ...
                        <Box paper sx={{ height: 300, overflow: 'auto' }}>
                            <Stack direction="column" spacing={1} sx={{ p: 1 }}>
                                {unselectedColumns.map((column: any) => (
